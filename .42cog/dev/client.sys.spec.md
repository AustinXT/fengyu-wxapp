...
顾客确认过的写操作（目前只有「取消订单」）成功后**就地落已知状态**（`status='已关闭'` + 停表 + 清两个到期标记），不等刷新回来才纠正 —— 刷新失败时页面继续挂着「取消 + 去支付」会诱导顾客再点一次，然后吃一条「订单状态不允许取消」。配套两道守卫：**写操作纪元**（写成功 +1；`_fetchDetail` 记下发请求时的纪元，响应回来发现纪元变了就丢弃 —— 在途那一发读的是写之前的快照，落盘会把「已关闭」画回「待支付」），以及**写返回后的 `_destroyed` 复查**（请求在途时页面被卸载，再 Toast/setData 就是往下一个页面冒陈旧提示、往死实例上写数据）。

⚠️ **不给写后刷新加「同步中禁用所有操作」的闸**：那是把回退掉的「本地判据封支付入口」换个名字加回来，刷新持续失败时顾客将无法操作。回款侧由 `order.repay` 事务内 `FOR UPDATE` 重读订单 + 重算欠款兜底，重复提交只会拿到 `INVALID_STATE`。
