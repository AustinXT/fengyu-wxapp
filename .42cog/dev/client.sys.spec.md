...
⚠️ **`card.js` 的候选 SELECT 与 order 侧口径不同**：它**带**了 `lakala_out_order_no IS NULL`（既有代码），因此有同型的漏关面 —— 顾客充值时被 `recharge` 的 existing 检查挡一次，可由 `order.list` 的懒清理自愈（可恢复的体验问题，非资金问题）。**`card.test.js` 的守卫锁只锁 UPDATE 侧，测不到这个 SELECT。** 已列跟进项。
...
| 隐藏期间跨过截止点 | 清 UI + **关支付入口**（不在此处发请求，紧随的 onShow 加载会刷） | 同上 |
| 非权威归零（旧云函数只给 `expire_at`） | 清 UI + 重载，**按订单号只一次** | `_fallbackZeroReloadedOrderId` |
| 墙钟跳变（任一方向） | 清 UI + 校准，**不关支付入口** | 一拍最多排 1 秒，观测间隔离谱（回拨 > 2s / 前跳 > 30s）就是时钟不可信，**不**等于截止点已过。往回跳会凭空延长倒计时，往前跳会把服务端还认可的单判成过期、封掉支付入口 —— 两个方向都得当作「需要校准」而非「已过期」 |
| 服务端说「已过期但没关掉」（`expire_unresolved`） | 清 UI + **关支付入口** + **排一次延迟刷新** | 页面写着「正在确认订单状态」，没人去确认这句话就是假的；挡路的支付意图是瞬态的，隔几秒服务端多半能关掉 |
...
前端拿到 `<= 0` 一律视为协议降级（旧版本云函数、或上述服务端降级），走**带一次性闸门**的重载路径 —— 绝不当成「服务端已经处理完了」而永不重载。有界性因此不依赖协议版本：非权威归零每单最多重载一次；权威路径的收敛由服务端补关保证（重载回来要么已关闭、要么降级成非权威再吃一次闸门），同一单最多两次。

倒计时归零后那次刷新**在成功之前**要关掉支付入口（`payBlockedByExpiry`）：此刻页面并不知道这单关没关，刷新又可能失败；继续显示「请完成支付 + 去支付」就是在承诺一件不知真假的事，顾客点下去只会拿到「订单已超时」。成功的详情刷新会解除它 —— **除非那次响应本身就是 `expire_unresolved`**（服务端明说「已过期、试过两次都没关掉」），那正是闸门该继续关着的形态。
...
时延扣减取**网络残差的一半**而非整段：整段会把上行也算进去，弱网上行慢时页面会比真实截止提前归零、**封掉一张还能付的单**（自从归零会关支付入口，过度扣减就不再是无害的保守方向了）。

`loadDetail` **必须如实返回成败**：调用方据此决定要不要重试。它一度把失败吞掉只返回 void，结果就是「墙钟回拨后校准成功也白打一发」和「支付确认轮询收尾那次刷新失败无人兜底」两个缺陷。两处「这次刷新必须成功、否则页面会停在不可信状态」的场景共用一个有界重试（5 秒一次，`onHide`/`onUnload` 清掉）：①墙钟回拨后的校准（回拨刻意不关支付入口，失败就没有恢复点）；②确认轮询收尾（轮询自己会在渠道终态失败时清掉 `lakala_out_order_no`，订单因此重新进入「会被自动关闭」的集合）。
