...
└──────────────┼────────────────────────────────────┼────────────────────────────┘
               ...
               ▼                                    ▼
┌──────────────────────────────────────────────────────────────────────────────────┐
│                        CloudBase 环境（单环境，两端共用）                          │
│                                                                                  │
│  ┌─────────────────────────────────────────────────────────────────────┐        │
│  │                        云函数层（Node.js）                            │        │
│  │                                                                     │        │
│  │  ┌────────────┐  ┌────────────┐  ┌────────────┐  ┌────────────┐   │        │
│  │  │  clientApi  │  │  staffApi  │  │  payNotify │  │  wsGateway │   │        │
│  │  │（顾客端接口）│  │（员工端接口）│  │（支付回调） │  │（实时推送） │   │        │
│  │  └─────┬──────┘  └──────┬─────┘  └─────┬──────┘  └─────┬──────┘   │        │
│  └────────┼────────────────┼───────────────┼───────────────┼──────────┘        │
│           │                │               │               │                   │
│           ▼                ▼               ▼               ▼                   │
...
│  │                    数据访问层（云函数内部）                             │        │
│  │   ┌─────────────────────────┐     ┌──────────────────────────────┐  │        │
│  │   │  PG 自托管数据库          │     │  Workfine SQL Server         │  │        │
│  │   │  （读写；小程序专属数据）   │     │  （只读；业务主数据）            │  │        │
│  │   │                         │     │  111.229.31.128:1433          │  │        │
│  │   │  ・orders / order_items  │     │  wkdb_20220804_86cd3292       │  │        │
│  │   │  ・service_orders/items  │     │                               │  │        │
│  │   │  ・appointments          │     │  ・UDT_S_287  员工档案         │  │        │
│  │   │  ・revenue_allocations   │     │  ・UDT_M_219  门店列表         │  │        │
│  │   │  ・product_spu/sku_map   │     │  ・UDT_M_1281 可售项目         │  │        │
│  │   │  ・client_wechat_users   │     │  ・UDT_M_1383 门店自定义项目    │  │        │
│  │   │  ・staff_wechat_users    │     │  ・UDT_M_341  院装产品         │  │        │
│  │   └─────────────────────────┘     │  ・UDT_S_311  顾客档案（只读）  │  │        │
│  │                                   └──────────────────────────────┘  │        │
│  └─────────────────────────────────────────────────────────────────────┘        │
│                                                                                  │
...
| 云函数 | 所在项目 | 职责 |
|--------|---------|------|
| `clientApi` | fengyu-client | 顾客端全部接口（按 `action` 字段路由） |
| `staffApi` | fengyu-staff | 员工端全部接口（按 `action` 字段路由） |
| `payNotify` | fengyu-client 或共享 | 微信支付异步回调（幂等处理） |

> 两端云函数分属不同小程序目录，但可共用同一 CloudBase 环境；payNotify 因微信支付回调 URL 固定，需独立函数。
...
App({
  onLaunch() {
    wx.cloud.init({
      env: 'fengyu-staff-env-id',  // 同一 CloudBase 环境或独立环境均可
      traceUser: true
    })
  }
})
...
> 两端可共用同一 CloudBase 环境（推荐 MVP 阶段），环境 ID 通过 `envQuery` MCP 工具查询确认。
