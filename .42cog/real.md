...
- **角色权限中间件**：仅 `position === '门店经理'` 的员工可执行开单、确认收款、营业额分配、关闭订单等管理操作；美容师只能操作 `assigned_staff_wf_id` 匹配自己的服务单和 `preferred_staff_wf_id` 匹配自己的订单。所有权限由云函数中间件（`requireManager`/`requireStaffBound`）统一校验，不依赖前端传参。
