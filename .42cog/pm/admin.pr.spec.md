...
订单明细导出对普通转换的负差额增加“转换差额转入储值卡”独立行，金额取关联的正向 `card_transactions`；该行商品属性为空、订单金额与实付为正数，“储值卡抵扣”等于该入账额、“现付”为 0，使每行满足“订单金额 = 储值卡抵扣 + 现付”，并使 `转出负数 + 转入正数 + 储值金入账正数 = 0` 可直接勾稽。这是导出勾稽归类，订单详情仍以独立资产流水展示。
...
| AC-22 | 顾客详情“家居产品”纳入部分支付订单，正确展示待提/已付/购买数量；部分支付按实收比例向下取整到整件，完全退款行不展示、全部提货行保留 |
