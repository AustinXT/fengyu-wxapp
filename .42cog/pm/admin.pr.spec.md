...
订单管理页另提供“回款明细”异步导出，一行对应「一条 `sale_order_payments` × 一个商品子项」（有 `sale_payment_item_receipts` 时按 receipt，否则按 `sale_items`，详见下文），覆盖首次支付、回款、储值卡抵扣、退款及所有状态，金额保留正负号。导出复用当前订单列表的状态、类型、市场、门店、支付方式、转换模式、储值卡抵扣、搜索词和日期条件；下单日期按订单筛选，款项发生日期按当前流水的 `paid_at` 逐笔筛选，无 `paid_at` 的未入账流水在有款项日期范围时不命中。

回款明细导出的前 34 列表头与列顺序必须与订单明细导出逐字一致（款项专属列一律追加在末尾），使回款块可直接粘贴到订单明细下方合成一张表，按品项/顾客求和统计当月消费；其中“业绩归属日期”列在回款行取该笔款项的归属日期，按该列分组即为正确的业绩月份。金额口径：“实付”＝该笔款项落到该商品子项的有符号实收（转换转出与退款为负），是唯一可跨行求和的金额列；“储值卡抵扣/现付”由款项金额与 receipt 合计还原后按实付比例分摊，恒满足“实付 = 储值卡抵扣 + 现付”；“订单金额/总数量/可用数量/单价”为 `sale_items` 快照，同一商品的多笔款项行上重复出现，不可求和。

没有商品子项记录（`sale_payment_item_receipts`）的款项不得因改用 INNER JOIN 而从导出中消失，按以下两条路径之一展开：

- **订单仍有 `sale_items` 且非寄存单**（内部单、receipt 机制上线前的历史销售单/转换单）：按 `sale_items` 展开成多行，商品明细列（推荐人、商品类型、品质一级/二级、商品明细、总数量、单位、可用数量、订单金额、单价、经营类型）的取值逐字沿用订单明细导出的 item 行口径（含“单位”缺 SKU 时按商品类型回退“盒/次”），使两段导出粘成一张表后同一商品的这些列完全一致。款项金额按各行应付（`sale_items.sale_amount`）权重分摊到各行，保证“实付”跨行求和仍等于款项金额。转换单需同时带上转出与转入两行，否则该类款项会退回占位行。
- **寄存单或订单无商品明细**（充值单、WorkFine 历史单）：输出一条商品列留空的占位行，标注“寄存回款（未按商品拆分）/储值卡充值/历史订单（无商品明细）/款项未拆分到商品”。寄存单**刻意不按商品扇出**：其平均 57.55 个商品行 × 人均 12.7 笔回款约合 486 万行，既超 Excel 上限又因金额列本就留空而对按品项求和无贡献。

上述两条路径共用三条金额留空规则，避免重复计数或与订单明细口径冲突：款项未入账（非“已支付”）、寄存单（`total_amount=0` 与 `received>0` 并存）、混合支付中被折叠的储值卡抵扣从行（其金额已计入同事件现金主流水的 receipt）。第三种情形下商品明细列照常填满，仅金额列留空。

营业额分配的销售提成列表与导出同样提供“下单日期/款项发生日期”两种日期口径，默认按下单日期；款项发生日期按当前回款行的 `paid_at` 逐笔筛选，不因同订单其他款项命中而带出全部回款。服务提成继续固定按服务日期筛选。销售提成导出在“支付时间”后另有“回款归属日期”列，口径与回款明细导出同源：首次支付随订单，其余取款项级归属日期并回退 `paid_at`；旧的订单维度分配（无 `sale_payment_id`）回退订单级。
