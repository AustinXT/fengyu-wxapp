...
营业额分配的销售提成列表与导出同样提供“款项归属日期/款项发生日期/下单日期”三种日期口径，**默认按款项归属日期**；这两种款项口径都按当前回款行逐笔筛选，不因同订单其他款项命中而带出全部回款。服务提成继续固定按服务日期筛选。销售提成导出在“支付时间”后另有“回款归属日期”列，口径与回款明细导出同源：首次支付随订单，其余取款项级归属日期并回退 `paid_at`；旧的订单维度分配（无 `sale_payment_id`）回退订单级。
