...
**进销存三级权限与金额口径**：进销存只由专用的供应链总部、市场、门店角色或超级管理员访问，既有 manager / finance / product 等角色不自动获得库存权限。总部 scope 仅代表总部库存；市场 scope 代表本市场及下属门店；门店 scope 仅代表本门店。一次动作要求的权限必须来自同一角色绑定。供应链只看供应成本与市场结算价，市场只看本市场进货价、门店结算价及员工购价，门店不返回金额字段。供应链 SKU 的市场结算价默认按“核算价 × 市场折扣”计算，手工覆盖必须留原因；明细金额和单头汇总由数据库统一维护，`market_id` 由来源/目标主体自动派生。跨市场调货出库归来源市场、入库归目标市场，总部不因组织父级自动获得可见性。供应链员工购只能扣减总部供应链 SKU，员工须属于所选总部且不在任何市场链路中，按市场结算价计入独立库存单据，不计入门店营收。
