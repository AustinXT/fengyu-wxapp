...
订单明细导出必须覆盖筛选结果中的全部订单。WorkFine 历史订单或异常原生订单没有可导出的 `sale_items` 时，按订单级生成一条“无商品明细”兜底行，保留订单、顾客及金额字段且不虚构商品分摊；存在正常商品明细的订单不得重复生成兜底行。

订单明细导出对普通转换的负差额增加“转换差额转入储值卡”独立行，金额取关联的正向 `card_transactions`；该行商品属性为空、订单金额与实付为正数，“储值卡抵扣”为 0，“现付”按同额负数记账，使 `转出负数 + 转入正数 + 储值金入账正数 = 0` 可直接勾稽。该资产转入合成行是普通商品行“实付 = 储值卡抵扣 + 现付”规则的明确例外；订单详情仍以独立资产流水展示。
