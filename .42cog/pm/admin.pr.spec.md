...
- **离职员工（`is_resigned = true`）不可登录，且既有会话立即失效**（#318）：
  `login` 与 `getSessionFromCookie` 两处都过滤 —— 只挡 `login` 不够，JWT 有效期 24h，
  已登录的人在被标离职后还能继续用一整天。失败文案必须与密码错误**逐字相同**
  （否则「这个人已离职」成了可探测信息）。
...
**改挂父节点的两道守卫**（#318）：改挂走事务 + `org_nodes:reparent` advisory lock，锁内做两件事 ——
...
不自洽则回滚并列出前 5 个人的姓名，要求先调整他们的归属。判定口径与员工侧**同一个**
...
**「至少保留 1 个活跃超管」的守卫散落在三个入口**（#318）：`updateEmployee` 标离职、
`deleteEmployee` 物理删除、这里的 `revokeRole` 撤超管。三者必须取**同一把**
`admin:active_count` advisory lock —— 只把计数查询塞进事务不够串行（READ COMMITTED 下
两笔并发操作分别针对 admin A、B 时各自都读到 `count = 2`、改的又是不同行，双双提交 → 零管理员，
系统锁死），事务化反而**放大**窗口。`revokeRole` 的取锁 + 计数 + DELETE + 撤销审计在同一事务内；
撤非超管角色不取锁（不影响该计数）。撤自己的超管角色照旧直接拒。
