...
订单管理页另提供“回款明细”异步导出，一行对应「一条 `sale_order_payments` × 一个商品子项（`sale_payment_item_receipts`）」，覆盖首次支付、回款、储值卡抵扣、退款及所有状态，金额保留正负号。导出复用当前订单列表的状态、类型、市场、门店、支付方式、转换模式、储值卡抵扣、搜索词和日期条件；下单日期按订单筛选，款项发生日期按当前流水的 `paid_at` 逐笔筛选，无 `paid_at` 的未入账流水在有款项日期范围时不命中。
...
内部单、寄存单、充值单、WorkFine 历史单与未入账款项在库内没有商品子项记录，一律输出一条商品列留空的兜底行并标注“款项未拆分到商品/储值卡充值/历史订单（无商品明细）”，不得因改用 INNER JOIN 而丢失这些款项。兜底行在下列三种情形金额列必须留空，避免重复计数或与订单明细口径冲突：款项未入账（非“已支付”）、寄存单（`total_amount=0` 与 `received>0` 并存）、混合支付中被折叠的储值卡抵扣从行（其金额已计入同事件现金主流水的 receipt）。
