...
`sale_order_payments.performance_attribution_date` **恒有值**（迁移 0039 起由数据库 trigger 保证）：首次支付镜像订单级归属日期，同次混合支付的储值卡抵扣跟随主流水，其余款项入账时按 `paid_at` 初始化；未入账期间先按 `created_at` 占位、入账那一刻按 `paid_at` 重算，已人工调整过的不再重算。订单级归属日期被修改时，首次支付流水与同次储值卡抵扣流水一并同步（admin 与员工端两处入口同语义）；一次调整机会仍只记在 `sale_orders` 上，首次支付流水不可单独修改。
...
> 业绩类指标统一按 `performance_date` 聚合。**2026-09-14 订正（#137 / #140）**：原文「首次收款跟随订单归属日，后续回款/退款按真实流水日」「实际收款金额仍按 `paid_at`」**均已失效**——迁移 0040 起 `performance_date` 一律直读 `sale_order_payments.performance_attribution_date`，首次支付/回款/退款同口径，回退只发生在写入侧 trigger；工作台的实付/退款（`todayPaidAmount` / `todayRefundedAmount` / `yesterdayPaidAmount`）也已统一到归属日期。品项/生美按行级业绩事件，销售分配/提成跟随关联款项的业绩事件。服务实耗/服务提成仍按 `service_date`（不在归属日期口径内）。
>
