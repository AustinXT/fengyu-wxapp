...
- `sale_order_payments` 为回款、储值卡抵扣和退款增加 `performance_attribution_date`、`performance_attribution_adjusted_at`、`performance_attribution_adjusted_by`；入账时由数据库按 `paid_at` 上海自然日初始化，待支付/待审批且无 `paid_at` 时保持 NULL。同订单、同精确 `paid_at`、同状态的首次支付/回款与储值卡抵扣视为同次混合支付，卡流水继承主流水归属字段。
