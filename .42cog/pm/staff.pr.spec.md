...
#### 3.19 取货单
家居产品分次提货：选已购或转换换入的实物 → 选数量（≤ 当前已付且尚未提取的整件数）→ 查看本次自动出库的库存商品及数量 → 确认。转换单转入行与购买行同权，其“已付”按转出旧卡价值加本单净到账折算，差额未结清时按比例逐件释放。部分支付订单随实收增加逐件释放可提数量，后端须在事务内锁定销售明细并再次校验上限。员工不选择库存 SKU；系统优先读取下单冻结组成，历史空快照读取最新有效组成，并按“每件组成数量 × 提货数量”整套扣减。任一组成库存不足时整笔回滚。员工端新建销售单、转换转入单和寄存单时，家居商品未配置组成必须阻断。
