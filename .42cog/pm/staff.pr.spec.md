...
> **日期筛选口径（#139，2026-09-14）**：销售提成 Tab 按**款项业绩归属日期**
> （`sale_order_payments.performance_attribution_date`，与 admin 营业额分配默认口径 `attribution` 一致），
> 服务提成 Tab 按服务日期。管理层视图口径单一，**不提供 admin 那样的三档口径切换下拉**。
> ⚠ **筛选键与排序键不同是有意的**：排序仍按到账日（`paid_at`，资金事实、不可变），
> 筛选按归属日期（业绩口径、可人工调整 ±7 天）。所以会出现「卡片显示的到账日不在筛选区间内」，
> 这不是 bug。筛选器下方的口径提示文案即为此而设。
...
**业绩归属日期**: 员工端店长可在本店订单详情调整一次；系统管理员、财务仍可在 admin 订单详情调整。可选范围为原始订单上海自然日前后 7 天（含边界），无操作时限；成功后展示调整人/时间并写入操作日志。
**列表筛选**: 顾客姓名/手机号检索 + 全量订单状态下拉 + 起止日期；日期默认不限范围
**排序与分页**: 按 `sale_order_datetime` 倒序；`pageSize=20`，滚动触底加载

> **日期筛选口径（#139，2026-09-14）**：按**款项业绩归属日期**筛选——订单只要存在任一笔
> 归属日期落在区间内的**已入账**款项即入选（EXISTS 半连接，与 admin 订单管理默认口径
> `attribution` 同构）。管理层视图口径单一，**不提供 admin 那样的三档口径切换下拉**。
> ⚠ 同营业额分配：**筛选键与排序键不同是有意的**——排序仍按下单时间 `sale_order_datetime`，
> 卡片上展示的也是它，所以会出现「卡片日期不在筛选区间内」，不是 bug。
> ⚠ 附带后果：**0 笔已入账款项的订单**（WorkFine 历史单、纯待支付单、未收款即关闭的单）
> 在本口径下查不出来；部分支付单有已支付的首次支付行，仍可入选。空态已就此给出提示文案。
...
**家居产品列表**: 常规顾客详情在“疗程卡”后新增只读 Tab，跨门店汇总该顾客已支付/部分支付/已完成订单中的家居产品（含**购买行**与**转换单转入行**，转入行与购买行同权，口径与疗程卡侧一致）；展示产品名称、待提/已付/购买数量、状态、购买门店、购买时间和订单号。已付数量按单行实收占销售金额的比例折算并向下取整到整件（零销售额视为全部已付），待提数量不得超过“已付整件数 - 实际提货数”，同时扣除已退款结算数量。完全退款行不展示，已全部提货行保留；提货操作仍走既有订单提货流程。
...
#### 3.19 取货单
家居产品分次提货：选已购或转换换入的实物 → 选数量（≤ 当前已付且尚未提取的整件数）→ 查看本次自动出库的库存商品及数量 → 确认。转换单转入行与购买行同权，其“已付”按转出旧卡价值加本单净到账折算，差额未结清时按比例逐件释放。可提件数与折抵额度共用「剩余已付」口径（行实收 − 已提货金额 − 已转走金额，详见 backend.pr.spec）。部分支付订单随实收增加逐件释放可提数量，后端须在事务内锁定销售明细并再次校验上限。员工不选择库存 SKU；系统优先读取下单冻结组成，历史空快照读取最新有效组成，并按“每件组成数量 × 提货数量”整套扣减。任一组成库存不足时整笔回滚。员工端新建销售单、转换转入单和寄存单时，家居商品未配置组成必须阻断。
...
> 历史勘误：本表早期版本曾将「报货流程」「库存」列为待定/暂不实现，现已被进销存 v3 推翻
> （见 `docs/changes/arch/011_inventory-domain-v3.md`）。员工端现状：门店层库存已实现——
> 门店报货、分院配货收货、分院间调货（限同市场）、院顾客退货、院退货、院顾客产品出库、
> 院产品报损、分院库存盘点等 11 类可见 / 6 类可建 / 2 类可收货单据（#350 起院顾客产品出库只能由
> 提货 `order.createPickup` 产生、不可通用建单），经 staffApi
> `inventory.*` 15 个 action 办理（#352 起门店盘点可在小程序办理：盲盘录实盘数，账面数提交时由服务端记入；
> #348 起门店报货可存草稿：`createDoc(draft)` / `updateDraft` / `submitDraft` / `deleteDraft`，草稿不进市场汇总与分院配货，提交即终态）；门店侧响应不含任何金额字段（说明.md §8.1/§9.5），
> 市场/总部层业务仅在管理后台办理。
