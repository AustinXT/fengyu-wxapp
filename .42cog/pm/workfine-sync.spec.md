...
**手动维护字段（不来自 WorkFine）**: `skills`（技能标签数组，由 admin 后台维护；staff/client 端只读），同步时不覆盖
...
### 9.8 销售单/转换单/回款单在 WorkFine 中的存储方式与 PG 映射

> ⚠️ **2026-07-13 连库订正**：本节早期版本称"销售/回款/转换/退款共用 `UDT_S_209`、靠单号前缀区分"——这是**未经验证的错误假设**。实际生产库（`wkdb_20220804_86cd3292`）验证为**三张独立主表**，退款单**不存在**（明细无负实收、全库无独立表/前缀）。`UDV_273` 视图把三者 UNION 起来（末列 `UDF_V_18576` 硬编码类型字面量），是定位转换单/回款单来源表的关键线索。

| 单据 | 真实存储（主表 + 明细） | 单号前缀 | 顾客编号字段 | 主单量 |
|------|------------------------|---------|------------|--------|
| 销售单 | `UDT_S_209` + `UDT_M_213` | `FY-XSD` | `UDF_S_1485` | 85941 |
| 转换单 | `UDT_S_570` + `UDT_M_704` | `FY-ABZH` | `UDF_S_1485` | 18643 |
| 回款单 | `UDT_S_261` + `UDT_M_263` | `FY-HKD` | **`UDF_S_1488`** | 9879 |
| 退款单 | **不存在**（明细无负实收，全库无独立表） | — | — | 0 |

> 三表核心字段（`UDF_S_372` 单号 / `350` 日期 / `348` 市场 / `349` 门店 / `370` 姓名 / `507` 收款合计）**同名同义**，可直接 UNION。回款单额外有 `UDF_S_917`（原销售单/转换单号引用）。`UDF_S_507` 在三表都 = 明细实收聚合 = 顾客实付（销售单收款合计 / 转换单补差价 / 回款单补交欠款）。
>
