 ...
 *
 * 用例：
 *   1. happy: 已 login 的用户 bindPhone({phoneNumber}) → phone 写入 + updatedOrdersCount=0
 *   2. 补全历史订单：预插一条 client_phone=X / client_user_id=NULL 的 sale_order，
 *      bindPhone(X) 后 updatedOrdersCount=1 + sale_orders.client_user_id 被回填
...
async function casePhoneOccupied() {
  ...
  const res = await invokeAs(TEST_OPENID, 'auth.bindPhone', { phoneNumber: PHONE_OCCUPIED })
