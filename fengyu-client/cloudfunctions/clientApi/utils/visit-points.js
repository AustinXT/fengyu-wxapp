...
async function logVisitPointsFailure(client, so, rewardAmount, externalRef, err) {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO operation_logs
         (operator_employee_id, operator_name, operator_role,
          action, target_type, target_id, detail, source, created_at)
       VALUES (NULL, NULL, NULL, 'points.visitGrantFailed', 'service_order', $1, $2::jsonb, 'clientApi', NOW())`,
      [so.service_order_id, detail]
    )
...
async function grantVisitPointsSafe(client, so, items, now) {
  ...
  try {
    ...
    await logVisitPointsFailure(client, so, rewardAmount, externalRef, err)
