...
- [x] T2 — B-1 会员数历史化（S，半天）— **2026-04-25 完成**
- [x] T3 — B-2 员工数历史化（M，1-2 天）— **2026-04-25 完成**
- [x] T4 — B-3 门店数历史化（M，1 天）— **2026-04-25 完成**
- [x] T5 — B-4 保有会员历史化（**方案 B 实时计算**，S，半天）— **2026-04-25 完成**
- [x] T6 — C 派生指标分母切换（M，1 天）+ 移除阶段 1 过渡角标 — **2026-04-25 完成**

✅ **2026-04-25 收官**：阶段 1（文案对齐 + 过渡角标）+ 阶段 2（T0/T2/T3/T4/T5/T6）全部完成。
- 后端：`mgmtDashboard.summary` 接口 `storeCount` / `employeeCount` 字段升级为 `{ day, month }` 双口径；`monthlyAvgPerStore` 用 `storeCount.month`（月末分母）。
- 前端：`SummaryData` / `buildDisplay` 同步双口径；屏幕展示卡用 `day` 截面，月度派生分母用 `month`；阶段 1 过渡角标已从 wxml/wxss 移除。
- metrics.md 派生指标章节明确两套分母语义；变更记录追加 T6 完成行。
- 单测：mgmt-dashboard.test.js 86 用例全绿（包含双口径区分、`lastDayOfMonth` 边界用例 4 项 = 跨年/平年二月/月初/月末输入）；全套 724 通过。
