...
>    共约 6~7 天命中；最坏 3/31 平年在营收持平时虚增约 +10.7pp。
>    **不得改成「按日均折算」**——那是拿估算值冒充实际值，与本节「数据缺失一律 `--`、
>    不做估算填补」相悖，且换算后的数字无法对账到任何一天的真实流水。
> 2. **本周 / 自定义的同比基期跨闰年**（`lastYear`，既有，**尚未修复**）：区间跨 2 月底时，
>    去年同区间可能多/少含一个 2/29。2023–2027 逐日扫描命中 **5/1826 天**（+1 天 2 次、
>    −1 天 3 次），窗口极窄但**量级达 12.5%**（7 天当期比 8 天基期 → 日均持平也显示约 −12.5%），
>    是例外 3 的 8 倍。2028/2029 再触发。且此时同比基期的起止星期也漂了
>    （周六~周六 vs 周一~周日），「去年同周」并非同一周。
> 3. **今年跨闰年**（`previous` = `lastYear`，既有，**尚未修复**）：当年平年而上一年闰年时，
>    基期含 2/29 而当期没有，基期**长 1 天**；当年闰年则反向短 1 天。2025 与 2029 各有
>    306/365 天命中（首例 2025-03-01：当期 60 天 vs 基期 61 天）。量级 1.6%，
>    下次触发窗口 2029。改它属同比口径变更（`year` 的环比与同比共用同一区间语义）。
>
> 例外 2、3 同源于 `addYears()` 的 2/29 归一化（JS 把 2023-02-29 归一化成 2023-03-01），
> 顺带导致「同比今日」在 2/29 当天比的是去年 3/1 而非惯例的 2/28。
