...
| **2026-09-23** | **analyst 增幅文案两条本地例外落地（#314，拍板）**，见文末基期章节「analyst 的两条本地例外」小节。<br>· **`rate` 型零基期不再算「算不出」**：百分点差值走减法、不需要非零分母，两道基期守卫改排在 rate 分支之后，`0% → 30%` 出 `+30.0pct`。此前 `previous === 0` 排在前面，**只藏涨不藏跌**（`30% → 0%` 照常出 `-30.0pct`）。⚠️ **已知代价**：`service_orders` 最早 2026-07-08 而新客入口走首单（回溯 2022-08），2022–2025 的 1,352 个新客到店恒为 0，故**凡同比基期新客的 90 天到店观察窗「未完全落在 2026-07-08 之后」者都是割点伪影**（完全早于=假零基期、跨线=低估；⚠️ 别写成固定失效日或「与割点重叠」，前者忽略同比平移 12 个月致 2027 上半年仍命中，后者漏掉最严重的「完全早于」），集团级到店率同比将显示约 `+86.7pct`。该代价在拍板时已量化告知并被接受（根因由 #289 跟踪），**不要据此回头推翻**。<br>· **伪持平并入「持平」**：`toFixed(1)` 舍成 `0.0` 的不再带符号输出 `+0.0%` / `-0.0%`，配色一并变灰；判据是印出来的那个数（`Number(scaled.toFixed(1)) === 0`）而非原始 delta。⚠️ 阈值随展示精度走，analyst 1 位、admin 数据中心 2 位、首页看板整数，**别互抄**。<br>· 同轮确立 **文案与配色同源**：tone 读实际渲染值，不再自己重算方向；没印出数字时才退而用两期差值补方向，而**四种成因（零基期 / 负基期 / 非有限入参 / 溢出）里只有负基期允许这么做**（#307 既定分叉不变），其余弃判置灰。<br>· 同轮把 AI 助手的 `assistant-answer.ts` `formatSignedRate`（与看板吃同一个 `kpi.delta` 的第二实现，曾各自判零、且不挡 `NaN`）**收敛到 `metric-delta` 的无前缀内核**，顺带修掉 #317 登记的 `NaNpct`。<br>⚠️ 决策 1（负基期「由负转正 / 未转正」矩阵）点名的 **analyst 侧尚未落地**，不在 #314 范围。 |
...
>
> **delta% 算不出的情形一律出 `--`**（对齐上方「数字格式化规则」的防除零条目）：当期为空、基期为空、
> 基期 `<= 0`、当期或基期非有限值（`NaN` / `±Infinity`）。
> 基期为负时 `(cur-base)/base` 符号会翻转 —— 2026-09-22 只读库实测（南昌梦祥店「本周」业绩）：
> 基期 −2,646.00、当期 +264.00（已从负回正），旧式算出 **−109.98%**，徽章渲染成红色下滑，方向恰好反了。
> 负基期的「增长率」无可读语义（分母的方向性已丢失），归入算不出。
>
> **⚠️ 本节的「基期 `<= 0` / 非有限值一律算不出」是跨站点规则，不只管 admin。**
> `fengyu-analyst`（独立部署的经营分析站，复用 admin 的库与认证）有一份**完全独立**的实现
> `src/lib/metric-delta.ts`——它与 admin 的 `data-center/comparison.ts` **没有目录共享、没有 snapshot 守护**，
> 当初就是因为无人登记这条口径而把同一个符号翻转缺陷重写了一遍（#307）。两边现已对齐「算不出」的判定，
> 但**展示层刻意分叉**，别当漏改去"统一"：
>
> | | 负基期数值 | 负基期徽章配色 |
> |---|---|---|
> | admin `comparison.ts` + `DeltaBadge` | `null` → `--` | 灰，弃判方向 |
> | analyst `metric-delta.ts` | 文案「无基数」 | **仍按 `current > prevYear` 判绿/红** |
>
...
>
> ⚠️ **口径差（登记，非缺陷）**：admin 的 `deltaPct` 对 `unit === 'percent'` 的 KPI **也走除法**
> （相对变化 `%`），analyst 的 `rate` 型走**减法**（百分点 `pct`）；且 admin 零基期恒 `--`、
> analyst `rate` 零基期出数。两站点指标集不重叠，不会对同一现象给出相反结论。
> 另「持平」这个字面量在 analyst `metric-delta.ts` 与 admin 侧**各有一份**（跨端共享目录已 veto），
> 两边字面量须一致，靠各自的文案锚定测试守护。
> ⚠️ admin 那一份落在 `src/lib/delta-display.ts`，**随 PR #321（#310 #315）一并落地**——
> 该文件在 `dev` 上尚不存在，#321 合并前别照着这行去找它。
>
> analyst 保留方向是有意的：由负回正是真实的向好信息，不该跟着倍数一起丢；且两站点指标集不重叠
> （admin 是门店业绩等，analyst 是新客漏斗首单/年度贡献），不会对同一现象给出相反结论。
>
...
> > 任何落在割点前的同比基期都是 0。放开后**集团级到店率同比会渲染出约 `+86.7pct` 的绿色徽章**
...
> > 同比基期落在 2026 年 1–6 月，**仍在割点之前**；「今年 / YTD」这类区间更会长期混入割点前月份。
> > 准确判据要写成**否定式**——设基期新客的到店观察窗为 `[entry_date, entry_date + 90 天]`：
> > 完全早于 2026-07-08 → 到店恒 0（**假零基期**，当前全部受影响区间都是这一形态）；
> > 跨 2026-07-08 → 到店被**低估**；完全晚于 → 干净。
> > 即**「观察窗未完全落在 2026-07-08 之后」就是伪影**。
> > ⚠️ 别写成「与 2026-07-08 重叠」——那只覆盖跨线那一种，把最严重的「完全早于」漏在外面。
...
>
> **再写第三个看板要算增幅时**：admin 内走 `deltaPct`，analyst 内走 `metric-delta.ts`，
> 新站点则把这条规则先抄进它自己的模块头——本节是唯一的单一真相源。
