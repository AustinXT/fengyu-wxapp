...
>
> **环比基期（上期）的长度 —— 必须与当期等长**（2026-09-22 登记，#283）：

| preset | 当期 | 环比基期（上期） |
|---|---|---|
| 今日 | [今天, 今天] | [昨天, 昨天] |
| 本周 | [本周一, 今天] | [上周一, **上周同一个星期几**] |
| 本月 | [月初, 今天] | [上月 1 号, **上月同一日**]（上月天数不足则截到上月末） |
| 今年 | [年初, 今天] | [去年初, 去年同日] |
| 自定义 | [start, end] | 紧邻前一等长区间 |
...
> 基期 `<= 0`。基期为负时 `(cur-base)/base` 符号会翻转（净业绩由 −6,104 回正到 +50,000 会算出 −919%），
> 负基期的「增长率」无可读语义，归入算不出。
