...
**悬念**：`issueCoupon` 的 fallback 分支（L431-434，既无 validDays 也无 validTo）默认 **365 天**，若销售在 UI 漏填会得到非预期的一年有效期。应在 `createTemplate` 增加"两种模式至少填一个对应字段"的校验。

**`createTemplate` 当前并未校验 validDays/validTo 的必填**（L205-278 只校验了 discountValue 和 validFrom/validTo 顺序）→ **这就是 §十二 所说的"有效期逻辑需确认"真正对应的问题**。
...
5. **修复 createTemplate 有效期必填校验** [逻辑变更，直接改]
   - 文件：`fengyu-admin/src/actions/coupons.ts:205-278`
   - 改动：
     ```ts
     if (data.validityMode === 'days' && !data.validDays) return { success: false, message: '领取后 N 天模式需填写有效天数' }
     if (data.validityMode === 'fixed' && (!data.validFrom || !data.validTo)) return { success: false, message: '固定时段模式需填写起止日期' }
     ```
   - 同步到 `updateTemplate`
   - 前端 `coupon-create-page.tsx:52-74` 已有部分校验，但缺少模式 → 字段对应校验，需补齐
   - 测试：单测 coverage
