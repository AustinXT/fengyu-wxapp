...
本仓已为此踩坑两次。本条因此先跳号到两线均未占用的 0043 落在 test 线上。

**2026-09-21（PR #204，test→main）再次改号到 0046。** 合并时 dev 线已把 0043/0044/0045
用掉，且 dev 库的 `max(created_at)` 已是 `0044_wooden_human_torch` 的 1789790016262 ——
比本条旧 when（1789786018613）更大。drizzle migrator 判「已应用」只比 journal 的 `when`
（`lastDbMigration.created_at < migration.folderMillis`）**不比 hash**，沿用旧 when 会让
dev 库**永久静默跳过**本条、缺两列。故改号到 0046 并把 when 抬到 0045 之后。
同一次合并里 #182 的 `waived_amount` 迁移从 0044 改到 0047。

改号只动 `idx` / `tag` / 文件名与 `meta/*_snapshot.json` 的 `id`/`prevId`；snapshot 由
`drizzle-kit generate` 以 0045 为基线重新生成（生成出的 DDL 与本条原文逐字一致，即基线正确的证明），
不手工编辑。

⚠ 本条**刻意保持非幂等**：下方回填是破坏性的，重跑会把已拆出的 `refunded_quantity` 重算成 0。
误重跑必须是显式失败（首条 `ADD COLUMN` 抛 42701 回滚整个事务）而不是静默数据损坏。
2026-09-21 实测 dev / prod 两库均未按旧 when 应用过本条。相邻的 0047 无回填，故写成幂等。

`payment-migrations-regression.test.js` 原有的 `idx === index` 守护相应改写为
「已发布段逐位对齐 + 新增条目严格递增」——守住原意（已发布迁移不可改号）的同时允许跳号；
本次合并后 0..45 段已无空洞，`PUBLISHED_THROUGH` 取 45。

## 历史数据回填

写在 0046 同事务内（drizzle migrator 把整个文件包进单事务，`RAISE EXCEPTION` 才能真正回滚）：
...
- db：`schema/order.ts`、migration 0046、`scripts/verify-quantity-split.js`（新增只读核对脚本）
