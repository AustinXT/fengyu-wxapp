...
> **适用范围已于 2026-09-21 收窄为「开单 + 顾客端下单」**：服务单一路改由
> [arch/013](013_service-order-staff-market-support.md) 定义（候选扩到门店所属市场的出差支援人员、
> 技能白名单四项、`staff.list` 新增 `scene` 参数）。本文下方「不给 `staff.list` 加过滤参数区分调用方」
> 的选型结论**仅对开单与顾客端仍然成立**——三个入口的业务口径在 2026-09-05 需求会后已分叉。
