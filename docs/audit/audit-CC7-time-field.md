# 审计报告：CC7 时间字段责任 / 时区一致性（横切收官 + v2 独立验证）

**审计时间**：2026-04-25 初审，2026-04-26 v2 独立验证
**域 ID**：CC7（横切检查域，跨 25 业务域归集）
**审计员**：claude-opus-4-7（v1），claude（v2 batch agent）
**最终版本**：v1+v2 合并版（v2 全量独立扫描确认后合并）

> 本报告是 25 业务域审计完成后对 CC7 横切域的**收官归集**，并经 v2 独立全量代码扫描验证。
> 范围横跨：
...
> 5. v2 独立验证：staffApi / clientApi / payNotify / refund-cascade.js 全量云函数层扫描（15+13+1+1 个路由文件）
...
| `confirmed_at` (appointments) | nullable | **未写入**（仅置 status） | **未写入**（v1）→ **已修复**（v2 确认）| — | — | — |
| `voided_at` (sale_allocations) | nullable | `NOW()` (admin) | **不用**（staff 硬 DELETE） | — | — | — |
| `voided_at` (service_commissions) | **schema 缺该列**（v1）→ **已加列**（v2 确认）| — | refund-cascade 写入（v2）| — | — | — |
...
- **v2 确认**：v2 全量 grep 云函数层，`toISOString().slice` 模式仍未修改，12 处以上副本仍散落（详见 P2-CC7-12）。
...
- **v2 确认**：v2 全量扫描 `staff.js`，确认该段代码仍未修改。
...
- **文件**：`db/schema/appointment.ts:35` 声明 `confirmedAt: timestamp('confirmed_at')`，`fengyu-admin/src/actions/appointments.ts` confirmAppointment 不写该列。
- **风险**：审计"店长何时确认预约"无数据来源；schema 列处于"声明无写入"漂移状态。
- **v2 确认（staff 路径已修复）**：`staffApi/routes/appointment.js:205` v2 验证已写 `confirmed_at = $1`（`updated_at = $1` 同值）✅，v1 P1-CC7-06 在 staff 路径已关闭。
- **遗留**：admin 路径（confirmAppointment）仍未写入，admin 单独 ticket 处理。
...
- **文件**：`db/schema/order.ts:214` 有 `voidedAt`；`db/schema/service-commission.ts` 仅 `is_void` 无 `voided_at`。
- **v2 确认（已修复）**：`db/schema/service-commission.ts` 已加 `voidedAt` 列，`staffApi/helpers/refund-cascade.js:71` 确认写入 `voided_at = NOW()` ✅。
- **修复状态**：schema 列 + cascade 写入均已就位，v1 P1-CC7-08 关闭。
...
- **v2 确认**：v2 全量扫描 `db/schema/*.ts`，确认混用仍未修复。
...
- **v2 确认**：v2 扫描 `auth.js`，确认仍未使用 `NOW()`。
...
#### **[P1-CC7-12-V2]** sale_orders.approved_at 孤儿列（v2 新发现）
- **现象**：`db/schema/order.ts:102` 声明 `approvedAt: timestamp("approved_at")`（备注"审批时间"），但 v2 全量 grep `approved_at` 在 staffApi 所有路由中仅见：
  - `staffApi/routes/order.js:1575`：写入 `sale_order_payment_details.audit_at`（正确）
  - `sale_orders.approved_at` 字段**未见任何 UPDATE** 写入（audit_at 下沉到 details 子表后，主表字段成为孤儿）
- **风险**：审计"退款何时被审批"在 sale_orders 主表层无数据来源，与 `sale_order_payment_details.audit_at` 双轨脱节。
- **建议**：确认 `sale_orders.approved_at` 是否已废弃（可加注释或通过迁移 DROP），或在 `approveRefund` 路由补写。

#### **[P1-CC7-13-V2]** formatDateTime 手动 +8h offset 与 PG 存储时区耦合
- **文件**：`fengyu-staff/cloudfunctions/staffApi/routes/appointment.js:15-26`
- **现象**：`beijing = new Date(d.getTime() + 8*3600*1000)` 假设 PG 存 UTC，若 PG TZ=Asia/Shanghai 则存储字面量已是北京时间，再加 8h → 错误的 +16h 显示。
- **风险**：PG 实例 TZ 变化会导致前端显示时间偏移 8 小时。
- **修复**：(L7) `formatDateTime` 改为直接依赖 PG 返回带时区信息的字符串，或在 SELECT 中加 `AT TIME ZONE 'Asia/Shanghai'` 转换。

#### **[P2-CC7-14-V2]** service_items INSERT 不传 created_at/updated_at（依赖 DB DEFAULT，风格割裂）
- **文件**：`staffApi/routes/service.js:208-224`：
  ```sql
  INSERT INTO service_items (service_item_id, sale_item_id, ...) VALUES ($1, $2, ...)
  ```
  未列出 `created_at`、`updated_at`，依赖 DB DEFAULT。
- **对比**：`service_orders` INSERT（`service.js:175-176`）显式传 `created_at, updated_at`（JS `now`）。
- **评估**：行为正确（DB DEFAULT now() 兜底），但风格不一致。无实质风险，P2 记录。

### 3.4 P2（代码质量 / 可维护）

#### **[P2-CC7-15]** dateStr / yearStart 等"导出字符串"在三端散落 ≥ 12 处副本（v2 确认仍存在）
- **v2 确认位置**（全量 grep，实测）：
  - `staffApi/order.js:504,2494`（sale_order + sale_item 序号）
  - `staffApi/service.js:770,42`（服务单号 + serviceDate 默认）
  - `staffApi/appointment.js:60`（todayOnly 过滤）
  - `clientApi/order.js:449,464`（订单号 + sale_item 序号）
  - `clientApi/card.js:253,267`（充值卡订单号 + item 序号）
  - admin（v1 记录）：`actions/orders.ts` 等 × 6
- **修复**：(L3) 抽 `helpers/datetime.js` 提供 `todayDateStr()` / `yearStartStr()` / `orderNoDateStr()` 等单一实现，全 staff/client/admin 引用。

#### **[P2-CC7-16]** computedAt 字段直传 `new Date().toISOString()` 给前端 (staff mgmt 端 5 处)
...
- **修复**：(L3) 改为 `dayjs().tz('Asia/Shanghai').format('YYYY-MM-DD HH:mm:ss')` 显式格式化。

#### **[P2-CC7-17]** 客户端 formatters / format.ts 用 `new Date(str.replace(/-/g,'/'))` iOS 兼容兼当时区翻译
...
#### **[P2-CC7-18]** cron-worker 调度器显式 timezone='Asia/Shanghai' 但底层 STEP 仍用 `CURRENT_DATE`（PG 时区耦合）
...
| confirmed_at | 不写 | **已写**（v2 确认✅）| — | — | — | admin 仍不写 | P1 |
| voided_at | sa: `NOW()`；sc: **已加列+写入**（v2✅）| sa: 硬 DELETE；sc: cascade ✅ | — | — | — | sc 已修复 | P1 |
...
- [ ] **CC8 WXML/Vant**：前端 formatDateTime 可能展示误差（P2-CC7-17）
...
| L0 schema | `db/schema/service-commission.ts` | 已加 `voidedAt: timestamp('voided_at')` ✅ | P1-CC7-08（已修复）|
| L0 schema | `db/schema/user.ts` 与其他表 | 决策：全部 `withTimezone:true` 或全部 naive；统一后跑迁移 | P1-CC7-10 |
| L0 schema | `db/schema/order.ts:102` | 确认 `approved_at` 是否废弃：若废弃加 `// @deprecated`；若保留，在 `approveRefund` 同步写 `approved_at = NOW()` | P1-CC7-12-V2（v2 新发现）|
...
| L3 staff appointment | `routes/appointment.js:15-26` | `formatDateTime` 改为 PG 带时区字符串或 SELECT 加 `AT TIME ZONE 'Asia/Shanghai'` | P1-CC7-13-V2（v2 新发现）|
| L3 cron | `fengyu-admin/src/cron/steps/close-expired-appointments.ts`（新增） | UPDATE appointments → 已关闭 | P1-CC7-07 |
| L3 cron | 既有 STEP `*.ts` | `CURRENT_DATE` → `(NOW() AT TIME ZONE 'Asia/Shanghai')::date` | P2-CC7-18 |
| L3 helper | 新建 `cloudfunctions/*/helpers/datetime.js` | 统一 dateStr / yearStart / monthStart 助手函数 | P2-CC7-15 |
...
| L9 frontend | `fengyu-*/miniprogram/utils/format*.ts` | dayjs.tz('Asia/Shanghai') 解析 ISO+offset 字符串 | P2-CC7-17 |
...
-- 6. confirmed_at 列实际写入率（应该 > 0%，v2 确认 staff 路径已写，admin 待核）
...
5. **confirmed_at 写入**（P1-CC7-06）：调用 staff confirmAppointment 后，断言 `confirmed_at IS NOT NULL`（v2 已确认）；admin 路径待单独验证。
...
- 修复成本：**M-L**
  - L0 一行 ALTER DATABASE + `voided_at` 列 ≈ S
  - L3 三端 dateStr / 时间窗口收敛 ≈ M（需穷举 12+ 处副本）
...
- [ ] 把 P2-CC7-15 helper 收敛纳入 staffApi / clientApi 共享 helper 重构 ticket
...
- [ ] `sale_orders.approved_at` 孤儿列处理：与业务方确认是否废弃，若保留则 `approveRefund` 路由补写（P1-CC7-12-V2）
- [ ] admin confirmAppointment 补写 `confirmed_at`（P1-CC7-06 遗留）
...
| `confirmed_at` (appointments) | 1 | 0 | staff ✅（v2 已修复）；admin 仍不写 | admin 孤儿 |
| `approved_at` (sale_orders) | 1 | 0 | **0（孤儿列，v2 新发现）** | **100% 孤儿** |
| `voided_at` (sale_allocations) | 1 | 0 | admin × 1 (`NOW()`) | staff 用硬 DELETE |
| `voided_at` (service_commissions) | **已加列** ✅ | — | **refund-cascade ✅（v2 已修复）** | schema 已修复 |
...
| staff cloudfunction | 30+ | 12+（v2 实测） | 0 | mgmt-dashboard 11 | mgmt-customer 3 + customer 3 | 0 |
...
## 附录：云函数层关键代码位置速查（v2 验证）

| 时间字段写入场景 | 文件 | 行号 | 状态 |
|-----------------|------|------|------|
| confirmed_at 写入 | `staffApi/routes/appointment.js` | 205 | ✅ v2 确认已写 |
| checkin_at 写入 + 幂等 | `staffApi/routes/appointment.js` | 261–264（写入），251–258（幂等）| ✅ |
| started_at 写入 | `staffApi/routes/service.js` | 268 | ✅ |
| completed_at 写入 | `staffApi/routes/service.js` | 454 | ✅ |
| paid_at confirmOffline | `staffApi/routes/order.js` | 919–926 | ✅ |
| paid_at payNotify | `payNotify/index.js` | 270 | ✅ |
| voided_at sale_allocations | `staffApi/routes/order.js` | 1120 | ✅ |
| voided_at service_commissions | `staffApi/helpers/refund-cascade.js` | 71 | ✅ v2 确认 |
| dateStr UTC 切片（❌ 未修复）| `staffApi/routes/order.js` | 504, 2494 | ❌ |
| dateStr UTC 切片（❌ 未修复）| `staffApi/routes/service.js` | 770, 42 | ❌ |
| dateStr UTC 切片（❌ 未修复）| `clientApi/routes/order.js` | 449, 464 | ❌ |
| dateStr UTC 切片（❌ 未修复）| `clientApi/routes/card.js` | 253, 267 | ❌ |
| formatDateTime +8h offset（⚠️ TZ 耦合）| `staffApi/routes/appointment.js` | 15–26 | ⚠️ v2 新发现 |
| last_login_at JS new Date() | `staffApi/routes/auth.js` | 143 | ⚠️ |
| sale_orders.approved_at 孤儿 | `db/schema/order.ts` | 102 | ⚠️ v2 新发现 |
| service_items INSERT created_at 缺失 | `staffApi/routes/service.js` | 208–224 | P2 v2 新发现 |
