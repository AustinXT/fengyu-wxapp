...
### 7.1 prod 发布欠账 —— 验收期间已被 v1.16.34 补上（admin 镜像仍未部）

> **本节已随事态更新。** 写下时 prod 落后 7 个迁移；2026-09-22 复核时库侧已全部到位
> —— 另一条线在此期间完成了 v1.16.34 发布（库 + 三个云函数）。原始判断与后续核对都保留在下表。

| 项 | 验收时（2026-09-21） | 复核（2026-09-22） |
|---|---|---|
| 迁移 | prod **0040**，待应用 7 条 | ✅ **待应用 0 条**，已到 0047 |
| 0041 视图直读 | ❌ 仍有 5 处 `paired_payment` | ✅ |
| `trg_sale_orders_sync_payment_attribution` | ❌ 不存在 | ✅ |
| `chk_sop_attribution_date_present` | ❌ 不存在 | ✅ |
| `sale_items.converted_quantity`（0046） | ❌ 不存在 | ✅ |
| **I6 首次支付归属日脱拍** | **34 条**（每天新增） | ✅ **0 条** —— 正如预判，被 0041 的①回填自动拉齐 |
| admin 镜像 | e0e7d734（09-12） | ⚠ **仍是 e0e7d734**，未随 v1.16.34 更新 |

**仍然成立的一条风险**：库已是 0046/0047 的新语义，而 **admin 还跑着拆列之前的旧代码** ——
`docs/changes/arch/012` 把这种「只部分部署」明确列为禁止状态（旧代码会把已退/已转份额
读回可提可退）。**营业前需补部 admin/analyst 镜像。**

原始记录：验收当时 prod 是「旧代码 + 旧库」自洽状态，没有在出错（I1/I2/I2b/I4/I5 全 0 违规），
34 条 I6 脱拍由旧视图的 CASE 回退兜着，不构成错账。

**新增 `db/scripts/preflight-release.js`**（只读）：一条命令给出目标库落后几个迁移、
待应用清单与逐条风险、5 项不变量现值、0041/0046 交付物是否到位，以及出自
`docs/changes/arch/012` 的执行顺序硬约束。对 prod 已跑通 —— 这次「发布是否真的生效」
的前后对比就是用它做的。

> 结论修订：本报告 §1 的「27 条达标」指**代码与 dev 环境达标**。
> 生产侧在 2026-09-22 已随 v1.16.34 补齐库与云函数；admin 镜像待部。
