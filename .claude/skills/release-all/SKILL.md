...
description: |
  ...
  cross-compiles the Next.js admin and ships it to the target host
...
metadata:
  ...
  version: 2.1.0
  title: 代码发版（dev/prod）
  description_zh: 预检门禁 + 目标库 db:migrate + admin 交叉编译发布 + client/staff 云函数发布 + 版本号更新（dev/prod 双环境参数化）
...
参数（可组合）：`dev` / `prod`（二选一，默认 prod）+ `skip-tests` `skip-version` `skip-admin` `skip-cloudfn`。数据库迁移是发布前硬门禁，**不提供** `skip-migrate`。

## Usage

`/release-all [dev|prod] [skip-tests|skip-version|skip-admin|skip-cloudfn]` — 手动触发（有部署副作用，不会被自动调用）。无环境参数 = prod 全流程；显式 `dev` = 发测试环境（ali-demo + 47.113.202.7）。无论目标环境，均在代码部署前执行目标库的 `db:migrate`。
...
- **发 prod 后必须恢复 dev env（§7）**，避免误留 prod 配置；发 dev 则保持 dev（dev 是安全态）。
...
## §5 Phase 4 — 云函数发布（除非 `skip-cloudfn`；**串行，禁止并行**）
...
5. **迁移结果**：复述 Phase 2 的 `db:migrate` 成功输出，并确认执行目标为 `$EXPECT_IP:5433/fengyu_wxapp`；不要在此阶段重复执行迁移。

---

## §7 Phase 6 — 恢复 dev env（防呆，仅发 prod 时）
