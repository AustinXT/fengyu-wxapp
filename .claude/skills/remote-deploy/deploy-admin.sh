...
IMAGE_REF="fengyu-admin:${ENV}-${APP_REVISION}-${BUILD_FINGERPRINT}"
REMOTE_RELEASE_DIR="$REMOTE_DIR/.deploy/releases/admin/$RELEASE_ID"

print_release_manifest admin "$ENV" "$LOCAL_BUNDLE" "$APP_REVISION" "$IMAGE_REF" "$RELEASE_ID"
...
build_image admin "$LOCAL_BUNDLE" "$IMAGE_REF" "$APP_VERSION" "$APP_REVISION"
