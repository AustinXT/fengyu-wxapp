...
1. 读取状态文件 `.claude/last-report.json`：

```bash
cat .claude/last-report.json 2>/dev/null || echo '{"commit": null, "date": null}'
```
...
pandoc "reports/凤御_进度汇报_YYYYMMDD.md" \
  ...
  --pdf-engine=xelatex \
  --metadata-file=".claude/skills/progress-report/templates/pandoc.yaml" \
...
| State | `.claude/last-report.json` | 上次汇报状态（commit + 日期） |
