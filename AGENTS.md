...
凤御双美容院微信小程序生态系统（monorepo）。各子项目详见其目录下的 AGENTS.md。
...
数据库迁移命令见 `db/AGENTS.md`，云函数部署见 cloudbase-deploy skill，小程序前端使用微信开发者工具打开。
...
cd .tree/feat/xxx && Codex             # 独立 Codex 会话
...
```bash
.agents/skills/remote-deploy/deploy-admin.sh <dev|prod>    # 本地 docker build → 传输 → compose up（按 env 自动路由：prod→fengyu-prod / dev→ali-demo）
```
