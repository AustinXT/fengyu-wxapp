 ...
 * 于是割点前入店的新客其到店数被截断，改动后**集团级到店率同比会渲染出约 `+86.7pct` 的绿色徽章**
 * （2026 年 3,156/3,639 = 86.7%）。量化依据见 issue #314 评论 `issuecomment-5788019145`。
 *
 * ⚠️ **别把失效日期写成某个固定日子**（本 PR 初稿写过「2027-01-01 前」，闸门 2 两谱系都揪出）：
 * 同比基期是当期平移 12 个月，所以 2027 年 1–6 月查询的同比基期落在 2026 年 1–6 月，
 * **仍受割点影响**；「今年 / YTD」这类区间更会长期混入割点前月份。
 *
 * 准确的判据要按**到店观察窗**分三态——设该窗为 `[entry_date, entry_date + 90 天]`
 * （`new-customer-funnel.ts` 的 `first_services` CTE，两端均为闭区间）：
 *   - `window_end < 2026-07-08` → 到店**恒为 0**，即**假零基期**（截至 2026-09 受影响区间
 *     全是这一形态：入客 2026-04-09 才是首个 `+90 天` 够得着割点的日子，同比对应 **2027-04-09**
 *     起才会出现下一种）
 *   - `window_start < 2026-07-08 <= window_end` → 窗内有一段没数据，到店**可能被低估**
 *   - `window_start >= 2026-07-08` → 干净
 * 合起来就是 **`entry_date < 2026-07-08` 的新客都不可信**（含割点当日的则是干净的）。
 * ⚠️ 别写成「与 2026-07-08 重叠」——那只覆盖了跨线那一种，把最严重的「恒为 0」漏在外面；
 * 也别写成「落在割点前的基期都是 0」——跨线那一种是低估不是归零。
 *
 * 这是**知情选择**：拍板人读过该数据后仍取本方案，理由是割点属全站问题、已由 #289 立案跟踪，
 * 不该由单个徽章承担。别再拿这份数据回头推翻本实现——那是已经否决过一次的建议。
 *
 * ⚠️ **第二类零基期与割点无关，割点补齐后也不会消失**（#314 评审，两谱系独立命中）：
 ...
 * 撞上 tone 的 `prevYear === 0` 弃判）这类自相矛盾的组合。
 * 没印出数字时（`rendered === null`）才退而用两期差值补方向，而那一族里**只有负基期允许这么做**
 * ——详见下一节末尾的四种来路。
 *
