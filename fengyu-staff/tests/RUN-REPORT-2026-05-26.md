...
- 原 `createTestSaleOrder`（fixtures.mjs）：`unit_real_price = totalAmount/quantity`（整卡价），
  与 `createTestSaleItem` 的 per-session 语义矛盾，且违反 [sale-items-money-fields]。
...
- **连带修复**：`smoke-order-conversion.mjs` 源卡 `totalAmount` 由 `500`（误填单次价）改为 `2500`
  （真实卡总额 = 5 次 × 500）—— 原依赖旧 helper "totalAmount/quantity=单次价" 的巧合，
  新语义下需填行应付总额。改后折抵 = 单次价 500 × 5 = 2500，链路正确。
- **验证**：全部 order/service/alloc/product 类 + 全套 53 smoke 通过（见末尾「最终结果」）。
...
4 个 Agent 并行各写 1 个新 smoke（只写 + `node --check`，不跑 e2e；run-all 自动发现）：

| 新文件 | 覆盖 action | 断言要点 |
|--------|------------|---------|
| `smoke-card-refund.mjs` | card.createRefund/approveRefund/rejectRefund | 退款 face vs pay 双口径、approve 扣 balance + card_transactions 落账、reject 不动账 |
| `smoke-order-pickup.mjs` | order.availablePickupItems/createPickup/pickupRecordsList | 家居产品分次自提：picked_up_quantity 原子累加、幂等、超提拒、记录列表（含自写 pickup_records cleanup） |
| `smoke-order-deposit-received.mjs` | order.updateDepositReceived | 寄存单历史实收 delete-rebuild 语义、received 重算、4 条边界（负值/非寄存单/非店长/越界 item） |
| `smoke-inventory.mjs` | inventory.list/detail | procurement 单据 scope 过滤可见 + 明细 + 非法 docCategory 拒（含自写 inventory_* cleanup） |
