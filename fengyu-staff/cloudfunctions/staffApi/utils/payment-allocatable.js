...
async function capturePaymentAllocatables(client, { salePaymentId, saleOrderId, eventAmount, directedItems }) {
  ...
  if (items.length === 0) {
    ...
    for (const d of perItem) {
      await client.query(
        ...
        [salePaymentId, saleOrderId, d.saleItemId, d.amount.toFixed(2), convCatMap.get(d.saleItemId) || null],
      )
    }
  ...
  if (Array.isArray(directedItems) && directedItems.length > 0) {
    ...
    // 最大余数法保证 Σ = evt 且每项非负。退款流水也会写 SPAI 供赤字分配展示，因此 prior 只统计正向已支付流水。
