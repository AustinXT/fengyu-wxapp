...
describe('#282 · 分页 SQL 的 ORDER BY 必须带唯一键 tie-break', () => {
  ...
  describe('第 2 层 · #282 修的 6 处逐条钉死（防被改回去）', () => {
    ...
    // 所以已修的这几处要把**完整子句**钉住。
    const EXPECTED = [
      ['customer.js', 'sop.created_at DESC, sop.id DESC',
        '顾客退款流水：sale_order_payments 的 created_at 同事务多行并列'],
      ['customer.js', 'created_at DESC, id DESC',
        '顾客手机号变更日志：operation_logs 批量写入同秒并列'],
      ['inventory.js', 'loc.location_type, loc.name, st.sku_name, st.batch_no, st.id',
        '库存批次：同库位同 SKU 同批次可以有多个 lot 行'],
      ['inventory.js', 'd.doc_date DESC, d.created_at DESC, d.id DESC',
        '单据列表：同日多单，doc_date 是 date 粒度'],
      ['order.js', 'pr.created_at DESC, pr.id DESC',
        '提货记录：一次提货可写多行，created_at 同微秒'],
      ['order.js', 'sop.created_at DESC, sop.id DESC',
        '门店退款流水：同上'],
    ]
    ...
    test('已确认安全的那些不许被「统一风格」删掉', () => {
      ...
      // 第 1 层因 `looksUnique` 对 `*_id` 一律放行、第 2 层清单又不含它们 → **全绿**。
      // 下面那条覆盖率断言就是为了让「新增查询却忘了登记」立刻可见。
      const SAFE = [
        ['customer.js', 'c.user_id ASC', 3,
        ...
        ['service.js', 'so.service_date DESC, so.created_at DESC, so.service_order_id DESC', 1, '服务单列表'],
        ['serviceCommission.js', 'so.service_date DESC, so.updated_at DESC, so.service_order_id DESC', 1,
          '服务提成待办（#239 修的）'],
      ]
    ...
    test('EXPECTED + SAFE 合起来覆盖全部 18 支分页 SQL（新增查询忘了登记会红）', () => {
      ...
      const pinned = new Set([
        // EXPECTED（本次 #282 修的 6 支）
        'sop.created_at DESC, sop.id DESC',
        ...
        'so.service_date DESC, so.updated_at DESC, so.service_order_id DESC',
      ])
      const unpinned = []
      for (const file of collectRoutes(ROUTES_DIR)) {
      ...
      expect(unpinned, `这些分页 SQL 没被 EXPECTED/SAFE 任何一张清单钉死:\n${unpinned.join('\n')}`)
        .toEqual([])
    })
