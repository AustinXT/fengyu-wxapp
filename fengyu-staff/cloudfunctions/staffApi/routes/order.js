...
WHERE sale_items.sale_item_id IN (
  ...
  -- 作用域 = 本单转出行引用到的**全部**源行，含 Δ_row=0 的已付清/overpay 行 —— 它们的
  -- sale_amount 同样被上面的 CTE 还原过，不重算就会留下偏高的 paid_sessions。
  -- ⚠ 但必须排除 sale_amount <= 0 的行：已全退、被 STEP 2.5 压成 paid_sessions=0 的 0 元赠品卡
  --   会命中公式里 sale_amount <= 0 THEN session_count 的兜底而重回满次数，
  --   而 paid_sessions 是「已退款卡从卡包消失」的唯一机制。
