...
Page({
  ...
  onShow() {
    ...
    //
    // 但同区间的重查**已经在路上**时就别再发一次：首屏那一两秒里切出去抄个手机号
    // 再切回来是高频动作，重复一次就是一次全区间扫描。在途那次同样承担权限重验，
    // 等它即可。与 setRange 的去重①、applyCustomRange 的同值早退保持对称
    // （这条是三个入口里最常触发的，之前恰恰漏了）。
    // 只认重查：翻页在途不算，那时仍需要一次真正的 reset。
    if (this.data.loading && this._inflightReset) return;
  ...
  buildSearchView(items: PerformanceItem[], keyword: string, total: number, limit?: number, offset?: number) {
    ...
    // 号码有「带 86」「不带 86」两种存法，关键词也一样 —— 候选必须让两种存法的**结果一致**，
    // 否则同一个顾客会因为历史数据的写法不同而时搜得到时搜不到，直接导出错误结论。
    //   `+86 139` → 候选 [86139, 139]：两种存法都能命中 ✓
    //   `8613`    → 剥完只剩 `13`，噪声太大不能要；但若只留 `8613`，
    //               就变成「存成 +86 的搜得到、存成国内格式的搜不到」——
    //               所以这种半截国际前缀**两边都不匹配**，等用户再输一位就对称了
    const intlPrefix = /^861[3-9]/.test(kwDigits);
