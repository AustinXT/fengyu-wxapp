...
Page({
  ...
  onShow() {
    ...
    //
    // ⚠️ 这里**刻意不做「同区间在途就跳过」的去重**（按钮与 picker 两个入口都做了）。
    // 试过，被评审否掉：回前台这一次请求承担的是**权限复验**，而在途那次是切出去**之前**
    // 发的 —— 它的鉴权发生在隐藏之前，隐藏期间员工完全可能被撤权/调店/手机号被置空。
    // 拿它顶替新的复验，等于把已经无权查看的薪酬继续留在屏幕上。
    // 多一次全区间扫描是这条安全线的代价，`_seq` 会保证只有最新响应生效。
