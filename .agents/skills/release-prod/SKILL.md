...
- `.Codex/skills/remote-deploy/deploy-admin.sh prod ali-demo` — admin 交叉编译 + 远程部署
...
- 若本次涉及 schema 变更：迁移到 5433 是**独立人工前置**（见 `db/AGENTS.md`），不在本技能内 —— 停下来告诉用户。
...
```bash
.Codex/skills/remote-deploy/deploy-admin.sh prod ali-demo
```
