...
1. **当前不在嵌套 worktree**
   ...
   ```
   若 `git-dir` 指向 `.Codex/worktrees/.../.git`，说明已在 worktree 内——**拒绝执行**，让用户先 `ExitWorktree` 回到主仓库。
...
2. **调用 `EnterWorktree`**：
   ...
   ```
   这会：创建 `.Codex/worktrees/wt-xxx-yyy/`、切新分支（与 worktree 同名）、把当前会话的 cwd 切到新 worktree。
...
把 `<...>` 占位符替换后直接发：
    ...
    【worktree 路径】 /Users/nv/proj.xt.com/fengyu-wxapp/.Codex/worktrees/<wt-name>
    ...
    【操作约束】
    1. 遵守项目根 AGENTS.md 与涉及子目录 AGENTS.md 的编码规范与"编码后自检"
...
```bash
git worktree remove .Codex/worktrees/"$WT_BRANCH"
...
2. 两条出路：
   - **手动解决**：`cd .Codex/worktrees/<WT_BRANCH>`，先 `git merge origin/$ORIG_BRANCH` 或 `git rebase $ORIG_BRANCH` 在 worktree 侧解决，解决完再触发本 skill 从 Phase 3 的 merge 步骤重来
