...
import { MOCK_PRODUCTS, MOCK_PRODUCT_SKUS, MOCK_PRODUCT_CATEGORIES } from "@/lib/mock-data"
...
export default function ProductDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const product = useMemo(
    () => MOCK_PRODUCTS.find((p) => p.productId === id) ?? null,
    [id]
  )

  const skus = useMemo(
    () =>
      MOCK_PRODUCT_SKUS.filter((s) => s.productId === id).sort(
  ...
  if (!product) {
    return (
      <div className="space-y-4">
        ...
        <p className="text-[var(--muted-foreground)]">商品不存在</p>
      </div>
    )
  }
