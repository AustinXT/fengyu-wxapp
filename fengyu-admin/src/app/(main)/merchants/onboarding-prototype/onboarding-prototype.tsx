...
export function OnboardingList({ applications, embedded = false }: { applications: OnboardingListItem[]; embedded?: boolean }) {
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link href="/merchants/onboarding-prototype/new"><Button><Plus />为门店发起入网</Button></Link>
        </div>
      </div>
                    ...
                    <tr key={application.id} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--muted)]">
                      ...
                      <td className="px-3 py-3">
                        <div className="flex justify-end gap-2">
                          ...
                          <Link href={`/merchants/onboarding-prototype/${application.id}`}><Button size="sm" variant="outline">办理<ChevronRight /></Button></Link>
                        </div>
                      </td>
                    </tr>
                  )
  ...
  function create() {
    ...
    startTransition(async () => {
      try {
        ...
        toast.success(result.message)
        router.push(`/merchants/onboarding-prototype/${result.id}`)
      } catch (error) {
...
export function OnboardingEditor({ application }: { application: OnboardingDetail }) {
  const router = useRouter()
  const [pending, startTransition] = useTransition()
