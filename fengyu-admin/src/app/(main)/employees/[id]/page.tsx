...
import { MOCK_EMPLOYEES, MOCK_PERMISSION_ROLES } from "@/lib/mock-data"
...
export default function EmployeeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const employee = useMemo(
    () => MOCK_EMPLOYEES.find((e) => e.employeeId === id) ?? null,
    [id]
  )

  const roles = useMemo(
    () => MOCK_PERMISSION_ROLES.filter((r) => r.employeeId === id),
    [id]
  )
  ...
  if (!employee) {
    return (
      <div className="space-y-4">
        ...
        <p className="text-[var(--muted-foreground)]">员工不存在</p>
      </div>
    )
  }
