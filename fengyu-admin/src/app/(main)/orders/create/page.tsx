...
import { MOCK_CUSTOMERS, MOCK_PRODUCT_CATEGORIES, MOCK_PRODUCTS, MOCK_PRODUCT_SKUS } from "@/lib/mock-data"
import type { Customer, Product, ProductSku } from "@/lib/types"
...
export default function OrderCreatePage() {
  ...
  const [selectedCategoryId, setSelectedCategoryId] = useState<string>(MOCK_PRODUCT_CATEGORIES[0]?.categoryId || "")
  ...
  const categoryProducts = MOCK_PRODUCTS.filter((p) => p.categoryId === selectedCategoryId)
              ...
              <CardContent className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {categoryProducts.map((product) => {
                    const skus = MOCK_PRODUCT_SKUS.filter((s) => s.productId === product.productId)
                    return (
