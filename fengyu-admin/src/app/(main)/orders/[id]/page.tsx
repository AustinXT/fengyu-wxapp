...
import { MOCK_ORDERS, MOCK_SALE_ALLOCATIONS, MOCK_OPERATION_LOGS } from "@/lib/mock-data"
...
export default function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  const order = MOCK_ORDERS.find((o) => o.saleOrderId === id)

  if (!order) {
    return (
      <div className="py-20 text-center text-[#999999]">
        <p className="text-lg">订单不存在</p>
        <Link href="/orders" className="text-[var(--primary)] hover:underline mt-2 inline-block">返回订单列表</Link>
      </div>
    )
  }

  const items = order.items || []
  const allocations = MOCK_SALE_ALLOCATIONS.filter((a) =>
    items.some((i) => i.saleItemId === a.saleItemId) && !a.isVoid
  )
