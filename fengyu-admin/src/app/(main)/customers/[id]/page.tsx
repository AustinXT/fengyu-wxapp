...
import type { SaleOrder, Appointment, SaleItem } from "@/lib/types"
...
export default function CustomerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const customer = useMemo(
    () => MOCK_CUSTOMERS.find((c) => c.userId === id) ?? null,
    [id]
  )

  const orders = useMemo(
    () => MOCK_ORDERS.filter((o) => o.clientUserId === id),
    [id]
  )

  const appointments = useMemo(
    () => MOCK_APPOINTMENTS.filter((a) => a.clientUserId === id),
    [id]
  )

  const activeSaleItems = useMemo(() => {
    const orderIds = orders.map((o) => o.saleOrderId)
    return MOCK_SALE_ITEMS.filter(
      (item) =>
        orderIds.includes(item.saleOrderId) &&
  ...
  if (!customer) {
    return (
      <div className="space-y-4">
        ...
        <p className="text-[var(--muted-foreground)]">顾客不存在</p>
      </div>
    )
  }
