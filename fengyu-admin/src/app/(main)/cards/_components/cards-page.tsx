...
/**
 * 判断当前行的状态（与后端 getCardsPaginated 保持一致的语义）：
 *  - expired:   有有效期且已过期
 *  - exhausted: 剩余为 0
 *  - active:    其它（含 expire_date 为空 = 永久）
 */
function computeCardStatus(row: AdminCard): 'active' | 'exhausted' | 'expired' {
  if (row.expireDate) {
...
export default function CardsPage({ cards, stores, orgNodes, total }: Props) {
  ...
  const filteredStores = useMemo(() => {
    ...
    const storeNodeIds = new Set(
      orgNodes
        .filter((n) => n.parentId === marketFilter && n.type === '门店')
        .map((n) => n.id),
    )
  ...
  const columns: Column<AdminCard>[] = [
    {
      ...
      cell: (row) => (
        <div className="flex flex-col">
          ...
          {row.clientPhone && (
            <span className="text-xs text-[#999999]">{formatPhone(row.clientPhone)}</span>
          )}
        </div>
      ),
    },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[var(--foreground)]">卡包管理</h1>
      </div>
