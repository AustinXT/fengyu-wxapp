...
import { MOCK_COMMISSION_RATES } from "@/lib/mock-data"
...
export default function CommissionPage() {
  ...
  const orderTypes = useMemo(
    () => [...new Set(MOCK_COMMISSION_RATES.map((r) => r.orderType))],
    []
  )
  const roleTypes = useMemo(
    () => [...new Set(MOCK_COMMISSION_RATES.map((r) => r.roleType))],
    []
  )
  const salesCategories = useMemo(
    () => [...new Set(MOCK_COMMISSION_RATES.map((r) => r.salesCategory))],
    []
  )

  const filterRates = (orgId: string) => {
    let result = MOCK_COMMISSION_RATES.filter((r) => r.orgId === orgId)
