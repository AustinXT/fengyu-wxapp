 ...
 *
 * 「调店不自动搬迁角色」这个决定成立的前提是**操作者当场看到需要跟进的角色** ——
 * 服务端已经把清单放进 `result.message`，但页面原先写的是硬编码
 * `toast.success("保存成功")`，把那条唯一的提示整个吞掉（codex 谱系第 3 轮指出，
 * 它当时的判断是「本 PR 的核心闭环在真实 UI 中仍未成立」，属合并阻断项）。
 *
