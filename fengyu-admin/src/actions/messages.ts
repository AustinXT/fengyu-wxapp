...
export const getMessagesPaginated = withPermission(
  ...
  if (filters.dateFrom) {
    ...
    conditions.push(gte(messages.createdAt, sql`${`${filters.dateFrom} 00:00:00`}::timestamp AT TIME ZONE 'Asia/Shanghai'`))
  }
  if (filters.dateTo) {
    conditions.push(lte(messages.createdAt, sql`${`${filters.dateTo} 23:59:59`}::timestamp AT TIME ZONE 'Asia/Shanghai'`))
  }
