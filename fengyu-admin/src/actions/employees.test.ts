...
describe('updateEmployee — 服务端输入校验 + 错误处理', () => {
   ...
   */
  /**
   * 事务体现在是：**advisory lock（`tx.execute`）→ 锁行重读（`FOR UPDATE`）** → admin 守卫
   * → `tx.update`（CAS）→ 角色查询/删除 → 各条审计。所以 tx 上还要有 `execute`，
   * 且 `select` 链要能接 `.for('update')`。
   * ⚠️ 锁序是 advisory **先于** 行锁 —— 与 `deleteEmployee` 同序，反了就是 lock ordering
   * inversion（`40P01`）。本文件另有一条用例（`order.slice(0,2)`）把这个顺序钉住了。
   *
...
describe('#249 调店不自动搬迁角色绑定 —— 只留审计 + 回传提示', () => {
  ...
  /**
   * 复职必须提示权限状态，且**实查**而非推断（codex 谱系第 5 轮 P1）。
   *
   * 本条验的是**正常路径**：走过事务化离职的员工角色确实被删光，复职后是角色真空。
   * ⚠️ 但「离职 ⇒ 角色真空」**不是可押注的不变量** —— 历史的两次独立提交遗留、
   * 以及 `sync-workfine.js` 改 `is_resigned` 不碰角色（至今有效），都会造出残留绑定；
   * 紧邻的下一条用例覆盖那个形态。所以实现是实查后分岔，不是假定真空。
   *
   * ⚠️ 这条用例上一版给已离职员工**伪造**了一条 `manager` 绑定当作「正常路径」，
   * 于是它验的其实是残留态 —— 正好避开了真实的角色真空，那次修复（把判据从 `||` 改成 `??`）
   * 也就没闭合任何东西：旧店查出来必然是空 → 落进 `no_binding_at_old_store` →
   * 返回干净的「员工信息已更新」。夹具必须先问「这个状态属于哪条路径」。
   *
