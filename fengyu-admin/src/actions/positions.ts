...
/** 全量查询职位列表 */
export const getPositions = withPermission('employee:list', async (): Promise<Position[]> => {
  const rows = await db
    .select()
    .from(positions)
    // 例外：sortOrder 手工排序权重
...
/** 查询启用中的职位（用于下拉选项） */
export const getActivePositions = withPermission('employee:list', async (): Promise<Position[]> => {
  const rows = await db
    .select()
    .from(positions)
...
export const updatePosition = withPermission(
  'employee:update',
  async (
    session,
    id: string,
...
export const deletePosition = withPermission(
  'employee:update',
