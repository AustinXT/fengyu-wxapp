...
function buildConditions(
  ...
  if (filters.startDate) {
    ...
    conditions.push(gte(cardTransactions.createdAt, sql`${`${filters.startDate} 00:00:00`}::timestamp AT TIME ZONE 'Asia/Shanghai'`))
  }
  if (filters.endDate) {
    conditions.push(lte(cardTransactions.createdAt, sql`${`${filters.endDate} 23:59:59`}::timestamp AT TIME ZONE 'Asia/Shanghai'`))
  }
