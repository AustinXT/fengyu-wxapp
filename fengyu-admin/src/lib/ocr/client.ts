...
function requiredEnv(name: string): string {
  ...
  if (!value) {
    throw new OcrConfigurationError(`OCR 未配置：缺少 ${name}`)
  }
  return value
}
...
function createClient() {
  const accessKeyId = requiredEnv('ALIYUN_OCR_ACCESS_KEY_ID')
  const accessKeySecret = requiredEnv('ALIYUN_OCR_ACCESS_KEY_SECRET')
...
export async function recognizeBusinessLicense(file: OcrFile): Promise<BusinessLicenseOcrResult> {
  ...
  const expiry = textValue(result, ['validPeriodEnd', 'validToDate', 'expiryDate', 'validTo', 'endDate', '有效期', '有效期截止日期'])
  const merRegAddr = textValue(result, ['address', 'businessAddress', 'registeredAddress', 'registerAddress', '住所', '注册地址', '经营场所'])
  const registeredRegion = getLakalaMerchantAreaPathByCode(lakalaMerchantAreaCodeFromAddress(merRegAddr))
  const parsed: BusinessLicenseOcrResult = {
    ...
    merBlisStDt: formatDate(textValue(result, ['validPeriodStart', 'validFromDate', 'startDate', 'validFrom', 'establishDate', 'registrationDate', '成立日期', '有效期起始日期'])),
...
export async function recognizeIdCard(file: OcrFile, side: 'face' | 'back'): Promise<IdCardOcrResult> {
  ...
  const parsed: IdCardOcrResult = side === 'face'
    ? {
        side,
    ...
    : {
        ...
        larIdcardLongTerm: period.longTerm,
      }
