...
describe('会员到店积分（admin）', () => {
  ...
  it('发放 SQL 的时间参数经 instantTs 落成 ISO 串，绑定层无 Date 实例', async () => {
    const execute = vi.fn().mockResolvedValue([{ points_balance: 120 }])
    ...
    // 片段复用守护：instantTs() 返回的同一个 SQL 对象在本条模板里被引用两次。
    ...
    expect(params).toEqual([
      ...
      '2026-08-13T10:00:00.000Z', 'client-001', '2026-08-13T10:00:00.000Z', 'client-001',
    ])

    // 下面两条锁的是**选型**（instantTs：绑 ISO+Z，毫秒不丢），不是 #253 的不变量：
    // 改成 nowTs() / beijingTs() 同样能修好 #253，但这两条会红。
    // 若哪天有意改选型，应同步改这两条断言，而不是删掉它们。
    // 毫秒必须保留：points_updated_at 是水位列，别的积分路径写 PG NOW()（微秒），
    // 这里若截断到秒，同一秒内的补发就是往回退。
    expect(params.filter((p) => p === '2026-08-13T10:00:00.000Z')).toHaveLength(2)
    expect(text.match(/::timestamptz/g)).toHaveLength(2)
  })
  ...
  it('业务锚点与余额变更时间可分离，points_updated_at 不跟着锚点倒退', async () => {
    ...
    await grantVisitPointsEntry(
      ...
      new Date('2026-09-22T02:00:00.456Z'), // 余额变更：北京 2026-09-22 10:00:00.456
    )

    const { params } = compile(execute.mock.calls[0][0] as SQL)
    // created_at 用锚点（出现 1 次），points_updated_at 用真实当下（出现 1 次，毫秒保留）
    expect(params.filter((p) => p === '2026-08-12T16:00:00.000Z')).toHaveLength(1)
    expect(params.filter((p) => p === '2026-09-22T02:00:00.456Z')).toHaveLength(1)
    expect(params.some((p) => p instanceof Date)).toBe(false)
  })
