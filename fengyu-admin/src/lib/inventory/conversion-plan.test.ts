...
import { allocateConversionLinks, summarizeConversion, suggestConversionUnitPrice } from './conversion-plan'
...
describe('库存转换成本守恒（#344）', () => {
  ...
  it('除不尽：100 拆 7 件，预填 14.29 的差额落在允许误差内；再多 1 分/件就超出', () => {
    ...
    expect(ok.tolerance).toBe(0.04) // ⌊0.5 × (7 + 1)⌋ 分
    expect(ok.balanced).toBe(true)
    // 人为改价 14.30 → 差额 0.10，超出 0.04
...
describe('库存转换血缘分摊（#344）', () => {
  ...
  it('分摊为 0 的配对不建关联（doc_links 要求 quantity > 0）', () => {
