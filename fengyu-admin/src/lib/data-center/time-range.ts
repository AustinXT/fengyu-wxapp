 ...
 *   year     ⚠️ 跨闰年 ±1 天（见例外 1）              同 previous
 *   custom   恒等长                                 ⚠️ 跨闰年 ±1 天（见例外 2）
 *
 * **两条已登记的既有偏差，均源自 addYears 的 2/29 归一化，都不在 #283 范围**
 * （#283 只修 week/month 的 previous）：
 *   例外 1 · year 的 previous/lastYear：当年平年而上一年闰年时基期含 2/29 而当期没有，
 *           基期**长 1 天**。2025 与 2029 各 306/365 天命中（首例 2025-03-01：当期 60 天
 *           vs 基期 61 天），反向（当年闰年）则短 1 天。量级 1.6%，下次触发窗口 2029。
 *   例外 2 · week/custom 的 lastYear：区间跨 2 月底时去年同区间可能多/少含一个 2/29。
 *           2023–2027 逐日扫描命中 5/1826 天（+1 天 2 次、-1 天 3 次），窗口极窄但
 *           **量级达 12.5%**（7 天当期比 8 天基期），是例外 1 的 8 倍；2028/2029 再触发。
 *           且此时 lastYear 的起止星期也漂了（周六~周六 vs 周一~周日），「去年同周」并非同一周。
 *
 * ⚠️ 别只盯 previous —— 上面例外 2 就是 #283 修复过程中连续四轮评审都漏掉的：
