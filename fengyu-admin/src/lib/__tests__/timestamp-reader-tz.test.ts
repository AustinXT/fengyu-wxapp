...
describe('parseTimestamp1114', () => {
  ...
  for (const tz of ['UTC', 'America/Los_Angeles', 'Asia/Shanghai']) {
    it(`TZ=${tz} 子进程下仍输出同一 UTC instant`, () => {
      const res = spawnSync('bun', [PROBE, LITERAL], {
      ...
      expect(res.stdout.trim()).toBe(EXPECT_ISO)
    })
  }
  ...
  it('对照：内置 new Date(字面) 在 TZ=UTC 下当 UTC 14:00（晚 8h，证明修复必要性）', () => {
    ...
    const res = spawnSync(
      'node',
      ['-e', `process.stdout.write(new Date(${JSON.stringify(LITERAL)}).toISOString())`],
      { env: { ...process.env, TZ: 'UTC' }, encoding: 'utf8' },
    )
    expect(res.stdout.trim()).toBe('2026-07-06T14:00:00.000Z')
  })
})
