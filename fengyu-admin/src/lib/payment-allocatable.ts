...
function allocateSignedCents(
  totalCents: number,
  rows: Array<{ saleItemId: string; weightCents: number }>,
): Array<{ saleItemId: string; amount: number }> {
...
export async function capturePaymentAllocatables(
  ...
  const ordRows = await tx.execute(sql`
    SELECT sale_order_type, legacy_source FROM sale_orders WHERE sale_order_id = ${saleOrderId}
  `)
  const ord = (ordRows as unknown as Array<{ sale_order_type: string; legacy_source: string | null }>)[0]
  ...
  if (items.length === 0) {
    ...
    for (const d of perItem) {
      const receiptId = await upsertReceipt(tx, {
        salePaymentId,
        saleOrderId,
  ...
  if (Array.isArray(directedItems) && directedItems.length > 0) {
    ...
    const priorMap = new Map(
      (priorRows as unknown as Array<{ sale_item_id: string; allocated: string | number }>)
        .map((r) => [r.sale_item_id, Number(r.allocated)]),
    )
    ...
    const addCents = (m: Map<string, number>) => {
