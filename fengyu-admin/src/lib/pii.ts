 ...
 */
export function sanitizeDetail<T>(input: T, inheritedKey?: string): T {
  if (input == null) return input
  if (typeof input === 'string') {
    return (inheritedKey ? maskByKey(inheritedKey, input) : input) as unknown as T
  }
  if (typeof input !== 'object') return input
  if (Array.isArray(input)) {
    return input.map((v) => sanitizeDetail(v, inheritedKey)) as unknown as T
  }
