 ...
 *   → 市场报货汇总 → 采购订单（市场行，#335 走供应链采购入库：分批入库/部分入库/超量拒）
 *   → 品项公司发货（赠送、无金额）→ 市场采购入库 → 分院配货（金额四件套）→ 院入库
...
const IMPL = path.join(__dirname, 'smoke-inventory-movements.impl.mjs')
...
const CONN = ensureInventoryE2eDb({ runDb: 'fengyu_inv_movements_e2e' })
