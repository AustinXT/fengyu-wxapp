#!/usr/bin/env bun
/**
 * 寄存单历史实收录入/编辑端到端冒烟（入口 wrapper）。
 *
 * 实现在 smoke-deposit-receipt.impl.mjs；本 wrapper 负责用 bun --preload 注入
 * next/cache + @/lib/auth + @/lib/permissions mock，并以 cwd=fengyu-admin/ 启动子进程。
 *
 * 用法：bun fengyu-admin/tests/e2e-actions/smoke-deposit-receipt.mjs
 */
...
const IMPL = path.join(__dirname, 'smoke-deposit-receipt.impl.mjs')
