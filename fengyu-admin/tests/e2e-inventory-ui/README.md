...
## 已知缺陷（详见 UX-FINDINGS.md）

| 级别 | 问题 | 影响 |
|---|---|---|
| P0 | 单据中心批次下拉永久卡在「加载库存批次...」 | 6 种需选来源批次的单据在后台**完全建不出来** |
| P0 | 员工购的员工下拉恒为空（递归 CTE 别名写错，5 处） | 市场员工购 / 供应链员工购**完全不可用** |
| P1 | 盘点单不记录账面数量（`stock_snapshot` 恒 NULL） | 盘点无法用于盈亏对账 |
| P1 | SKU「供货商」是自由文本，与供应商档案无外键 | 档案形同虚设，无法按供应商统计 |
| P1 | 业务错误被生产构建脱敏成英文占位或 digest 数字 | 用户不知道发生了什么 |
| P1 | 建单失败用 `alert()`、审批备注用 `prompt()` | 原生弹窗无法校验必填、阻塞页面 |
